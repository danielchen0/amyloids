...
\end{alignat}
Note that in the 01 pattern, the $1$s are attributed to positions as a bookkeeping convention (each $1$ marks a position that participates in a $\{C,Y\}$ adjacency), not to the physically emitting protein. The key claim is that the \textit{total} number of $1$s equals the total yellow fluorescence, which we verify: both rows sum to $6$.
