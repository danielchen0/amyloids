...
We note that on a path graph, the greedy matching (always fuse when possible from the left) is in fact a maximum matching of the completed static sequence. The standard exchange argument: let $G$ be the greedy matching and let $M^*$ be any maximum matching. If $|G| < |M^*|$, then the symmetric difference $G \triangle M^*$ contains an augmenting path with respect to $G$, i.e., a path that alternates $M^*$-edges and $G$-edges and starts and ends with $M^*$-edges. On a path graph the leftmost such augmenting path begins at some position $j$ with the edge $(j, j+1) \in M^*$, where $j$ is not covered by $G$. But then $j$ has no $G$-neighbor to its right (else greedy would have matched it), and in particular $\{s_j, s_{j+1}\} = \{A, B\}$ since $(j, j+1) \in M^* \subseteq$ (fusible pairs). Then greedy, scanning from the left, would have processed position $j$ either by matching $(j-1, j)$ (if available) or by matching $(j, j+1)$---a contradiction with $j$ being $G$-uncovered. Hence $|G| = |M^*|$, and the greedy and maximum matchings coincide on the path. Thus the sequential and static interpretations of the model are equivalent.
...
\]
(with $\cA(0, 0) := 1$ by convention) satisfies $F = 1 + z(2, 1) (I - z M(w))^{-1} (1, 1)^T$, which evaluates to
...
\end{equation}
where $a_\phi$ counts A-B-S patterns of length $\phi$ whose 01 representation is $0^{\phi-1} 1$, and $b_q$ counts patterns of length $q$ with all-zero 01 representation. Equation~(\ref{eq:s3-blockcount}) is the convolutional unfolding of~(\ref{eq:s3-bivar-gf}): writing $F(z, w) = B(z) / (1 - w\, A(z))$ with $A(z) = \sum_{\phi \ge 2} a_\phi z^\phi = 2 z^2 / (1 - 2z - z^2)$ and $B(z) = \sum_{q \ge 0} b_q z^q = (1+z)/(1 - 2z - z^2)$, one verifies
\[
...
\]
Equivalently, this is the specialization $\beta = 1 - \alpha$ of the Section 6.2 formula $2\alpha\beta(2-\alpha-\beta)/(1-\alpha\beta)$. At $\alpha = 1/2$, $R_{\text{SY}} \to (1/2)/(3/4) = 2/3$, recovering Section 2. By symmetry and the concavity of $\alpha(1-\alpha)$ (which both the numerator $2\alpha(1-\alpha)$ and the denominator $1 - \alpha(1-\alpha)$ depend on monotonically in opposite directions on $\alpha(1-\alpha) \in [0, 1/4]$), the limit is maximized at $\alpha = 1/2$, confirming that equal concentrations maximize the fluorescence yield. Numerically, at $\alpha = 1/4$ the limit is $2 \cdot \tfrac{3}{16} / \tfrac{13}{16} = 6/13 \approx 0.462$, in agreement with Monte Carlo simulation (see \texttt{split\_yfp\_unequal\_simulation.py}); the naive two-state Markov chain would instead yield $6/11 \approx 0.545$ at $\alpha = 1/4$, which fails the simulation check at every $\alpha \neq 1/2$.
...
\end{center}
These include the split-YFP, Sup35p, FRET, unequal-concentration, and
