...
\]
and $E_2(n)/n \to 4/9$. The unit-weight window is itself an idealization: a more physical model could weight a CFP--YFP pair at distance $d$ by a distance-dependent FRET efficiency rather than by a hard cutoff. The expected-value computation would still go through after replacing the indicator $d \le r$ with such a weight function, but the closed form would depend on the chosen geometry. Computing the full distribution for $r>1$ is substantially harder: adjacent indicators are correlated over a larger window, and a transfer matrix would need to track the last $r$ protein identities rather than only the rightmost state.
