...
\[
\#(n,k) := \binom{n-k}{k}, \quad k \leq \floor{n/2}
\]
Indeed, the number of ways to place $k$ non-adjacent objects in $m$ boxes is $\binom{m-k+1}{k}$ (see \cite{stanley2012}); setting $m = n - 1$ gives $\binom{n-k}{k}$ as claimed.
...
As a graph, the first 400 values appear in Figure~\ref{fig:splityfp}.

\begin{figure}[ht]
\centering
\includegraphics[width=0.75\textwidth]{SplitYFPamyloids.png}
\caption{$R(n)$ for split-YFP amyloids. The blue line is $2/3$ and the orange curve is the exact $R(n)$.}
\label{fig:splityfp}
\end{figure}
...
and the graph appears in Figure~\ref{fig:sup35}.

\begin{figure}[ht]
\centering
\includegraphics[width=0.75\textwidth]{splityfpandsup35.png}
\caption{$R(n)$ for split-YFP amyloids with Sup35p. The blue line is $1/3$ and the orange curve is the exact $R(n)$.}
\label{fig:sup35}
\end{figure}
...
The first 400 values of $R(n)$ appear in Figure~\ref{fig:fretonly}.

\begin{figure}[ht]
\centering
\includegraphics[width=0.75\textwidth]{FretOnly.png}
\caption{$R(n)$ for FRET YFP/CFP amyloids. The blue line is $1/2$ and the orange curve is the exact $R(n) = (n-1)/(2n)$.}
\label{fig:fretonly}
\end{figure}
...
The first 400 values of $R(n)$ appear in Figure~\ref{fig:fretsup35}.

\begin{figure}[ht]
\centering
\includegraphics[width=0.75\textwidth]{FretandSup35.png}
\caption{$R(n)$ for FRET YFP/CFP amyloids with Sup35p. The blue line is $2/9$ and the orange curve is the exact $R(n)$.}
\label{fig:fretsup35}
\end{figure}
...
\]
with fixed point $\pi^* = 1/\bigl(1 + 2\alpha(1-\alpha)\bigr)$. The expected fluorescence contributed by the $(n+1)$th protein is $2\alpha(1-\alpha)\,\pi_n$, so since $R(n) = 2E(n)/n$, at steady state:
\[
\lim_{n \to \infty} R(n) = 2 \cdot 2\alpha(1-\alpha) \cdot \pi^* = \frac{4\alpha(1-\alpha)}{1 + 2\alpha(1-\alpha)}
\]
which reduces to $4 \cdot \frac{1}{4} / \frac{3}{2} = 2/3$ when $\alpha = 1/2$. In the FRET setting with $P(C) = p_C$, $P(Y) = p_Y = 1 - p_C$, linearity of expectation gives $R(n) \to 2p_C p_Y$ directly, and with Sup35p at concentration $p_S$ (where $p_C + p_Y + p_S = 1$) the limit is likewise $R(n) \to 2p_C p_Y$. These generalizations would allow the model to be calibrated to experimental concentration ratios.
