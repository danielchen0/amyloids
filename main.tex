...
The convention is also compatible with a common static alternative. If one forms the path graph whose edges are adjacent unlike pairs and asks only for a maximum-cardinality matching, the leftmost greedy algorithm gives a maximum matching on a path: after taking the leftmost available edge, no maximum matching can use more than one edge among its two endpoints, and the remaining problem is again a shorter path. Thus the fluorescence count here agrees with the static maximum-matching count under unit weights. Other static ensembles, such as Boltzmann-weighted matchings or geometry-dependent pairing propensities, would define different null models and are outside the scope of the present calculation.
...
Our pattern becomes $\{C,Y,S \}^*$ where $C$ represents CFP, $Y$ represents YFP, and $S$ represents Sup35p. The 01 representation uses the same local FRET rule as Section~\ref{sec:fret}:
\[
f_i = \begin{cases}
0, & \text{if } i = 1, \\
1, & \text{if } i \ge 2 \text{ and } \{s_{i-1}, s_i\} = \{C, Y\}, \\
...
\end{cases}
\]
Thus every adjacency involving S contributes 0. The set of attainable 01 patterns is still $(0^* 0 1^*1)^* 0^*$, since the C-Y-only words from Section~\ref{sec:fret} already realize all binary strings beginning with 0. Unlike the two-letter case, however, a fixed 01 pattern no longer determines the protein word up to the first letter, because zeros may arise from equal C/Y neighbors or from the presence of S.
...
\qquad n \ge 2.
\]
As a small check, at $n=2$ the split-YFP-with-Sup35p and FRET-with-Sup35p distributions are both $(7,2)/9$ on $K_2=0,1$, giving variance $14/81$, exactly as the formulas above give. In the no-Sup35p split-YFP case, the $n=2$ distribution is $(1,1)/2$ and the $n=3$ distribution is $(2,6)/8$, giving variances $1/4$ and $3/16$, again matching the formula. The accompanying script \texttt{verify\_variance.py} also brute-force checks these variance formulas for small $n$.
...
\]
This is the usual central-limit scale for additive statistics of finite-state chains and transfer matrices. We do not need a normal approximation for the formulas above, since the exact finite-$n$ distributions or variances are available; nevertheless, for long amyloids a Gaussian approximation with these variances is the natural first approximation. For short amyloids, the exact distributions $P(n,k)$ should be used directly. For experimental ensembles of many fibrils, the relevant uncertainty also depends on the sampling design and the number of fibrils measured.
...
\]
and $E_2(n)/n \to 4/9$. If one uses the rough physical scale $r \approx 10$ suggested by the ratio of the CFP--YFP F\"{o}rster radius to the amyloid stacking distance, the same unit-window model gives
\[
\frac{E_{10}(n)}{n} \longrightarrow \frac{20}{9}
\]
in the equal-concentration FRET+Sup35p setting. This value can exceed $1$ because $R_{\text{F}}$ counts additive fluorescence units per protein, not the fraction of proteins that fluoresce. It also emphasizes that the nearest-neighbor value $2/9$ is an $r=1$ mathematical baseline rather than a quantitative experimental prediction.

The unit-weight window is itself an idealization: a more physical model could weight a CFP--YFP pair at distance $d$ by a distance-dependent FRET efficiency $\eta_d$ rather than by a hard cutoff. The expected-value computation would still go through, giving
\[
...
We have derived exact probability distributions, closed-form expected values, and finite-$n$ variances for fluorescence in four combinatorial models of amyloid fibrils. The principal expectation results are summarized below in each model's native units:
...
\end{center}
\noindent Here $R_{\text{SY}}(n) := 2E(n)/n$ is the fraction of proteins participating in split-YFP fluorescence (two proteins per fused pair), while $R_{\text{F}}(n) := E(n)/n$ is total FRET fluorescence per protein. The table places the four exact baselines side by side for reference, but the split-YFP and FRET ratios measure different physical quantities and are not directly comparable.
