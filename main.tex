...
\end{align}
These identities arise naturally from the fluorescence analysis but are of independent combinatorial interest. To our knowledge, the unweighted shallow diagonal sum $\sum \binom{n-k}{k} = F_{n+1}$ is classical, but closed forms for these $k \cdot 2^k$-weighted variants do not appear in the standard references.

As a corollary, their sum $S_1(n) + 2\,S_2(n) = \bigl[2^n(9n-6) + 6(-1)^n\bigr]/27$ governs the expected fluorescence in split-YFP amyloids, connecting these combinatorial identities directly to the biophysical application.
...
We can also verify the above via computer simulation (see \texttt{split\_yfp\_simulation.py}). A sample of computed values is shown in Table~\ref{tab:splityfp}, and the first 400 values appear in Figure~\ref{fig:splityfp}.
...
\hline
\multicolumn{6}{c}{$R(n) \to 2/3 \approx 0.666667$} \\
\end{tabular}
\caption{Sample values of $R(n)$ for split-YFP amyloids.}
\label{tab:splityfp}
\end{table}
...
\hline
\multicolumn{6}{c}{$R(n) \to 1/3 \approx 0.333333$} \\
\end{tabular}
\caption{Sample values of $R(n)$ for split-YFP amyloids with Sup35p.}
\label{tab:sup35}
\end{table}
