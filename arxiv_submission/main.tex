...
\end{align}
These identities arise here from the fluorescence analysis, but the sequences themselves are not new. The sequence $S_1(n)/2$ matches OEIS entry A127976, which arises as a convolution of Jacobsthal numbers, while $S_2(n)$ matches OEIS entry A095977, which counts $2 \times 2$ tiles in tilings of $3 \times (n+1)$ rectangles. Both sequences satisfy the same linear recurrence $a_n = 2a_{n-1} + 3a_{n-2} - 4a_{n-3} - 4a_{n-4}$ with different initial conditions.
