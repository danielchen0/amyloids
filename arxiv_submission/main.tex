...
\caption{$R(n)$ for split-YFP amyloids. The blue line is $2/3$ and the orange curve is the exact $R(n)$. The exponential correction term $O((1/2)^n/n)$ yields rapid convergence.}
\label{fig:splityfp}
\end{figure}
...
\]
where $\cA(n,k)$ is the combinatorial formula involving the $a_{\phi_i}$ and $b_q$ sequences with $\sqrt{2}$. This identity, which equates a sum over compositions weighted by products of irrational-valued sequences to a simple rational expression, serves as an independent verification of both derivations.
...
\caption{$R(n)$ for split-YFP amyloids with Sup35p. The blue line is $1/3$ and the orange curve is the exact $R(n)$. The exponential correction term $O((1/3)^n/n)$ yields even faster convergence than Figure~\ref{fig:splityfp}.}
\label{fig:sup35}
\end{figure}
...
This is the Binomial$(n-1, 1/2)$ distribution, which can also be seen directly: since the first protein determines the parity, the indicators $f_2, f_3, \ldots, f_n$ are mutually independent Bernoulli$(1/2)$ random variables (each adjacent pair is independently $\{C,Y\}$ or not), so their sum is Binomial$(n-1, 1/2)$.
